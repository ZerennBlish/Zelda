...
- Session 07 exception, 2026-09-15: Zerenn explicitly approved `unity_zelda` Pipeline `eval`/`run_script` for building and verifying Z-012, including its room and secret-cave connections. That build completed successfully. This task-specific approval does not change the general rule for future work.
- Session 08 exception, 2026-09-15: Zerenn explicitly approved Pipeline `eval`/`run_script` for Z-013 enemy removal, room connections, scene saving, and Play-mode verification. This is a task-specific exception; keep primitive-only results and the prohibition on Unity_ManageGameObject.
